package com.airbus.pocs3_eap.layout;
/*******************************************************************************
 ...
 * @since 2.0
 */
public CenterRowLayout (int type) {
	this.type = type;
}
...
protected Point computeSize (Composite composite, int wHint, int hHint, boolean flushCache) {
	...
	if (type == SWT.HORIZONTAL) {
		extent = layoutHorizontal (composite, false, (wHint != SWT.DEFAULT) && wrap, wHint, hHint, flushCache);
	} else {
...
protected void layout (Composite composite, boolean flushCache) {
	...
	if (type == SWT.HORIZONTAL) {
		layoutHorizontal (composite, true, wrap, clientArea.width, clientArea.height, flushCache);
	} else {
		layoutVertical (composite, true, wrap, clientArea.height, flushCache);
	}
}

Point layoutHorizontal (Composite composite, boolean move, boolean wrap, int width, int height, boolean flushCache) {
...
//				child.setBounds (childX, childY, childWidth, childHeight);
    ...
    //
    if (move) {
      Rectangle boundaryBounds = null;
      for (int i=0; i<count; i++) {
        if (boundaryBounds == null)
          boundaryBounds = bounds[i];
        else
          boundaryBounds = boundaryBounds.union(bounds[i]);
      }

      int dx = (width-marginLeft-marginRight-boundaryBounds.width)/2;
      int dy = (height-marginTop-marginBottom-boundaryBounds.height)/2;
